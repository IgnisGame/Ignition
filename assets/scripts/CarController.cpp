...
public:
	...
	// Camera dynamic offset (local x-axis)
...
public:
	void OnCreate() override
	{
		GetEntity().ForEachChild([&](ignis::Entity child)
			{
				if (m_has_cam) return;
				auto& tag = child.GetComponent<ignis::TagComponent>();
				if (tag.Tag == "Camera")
				{
					m_cam_entity = child;
					m_has_cam = true;
	...
	void OnUpdate(float dt) override
	{
		...
		if (glm::abs(steerInput) > 0.01f && speed > 0.3f)
		{
			...
			float highFactor = 1.0f - 0.45f * glm::clamp(
				(speed - SteerReduceAt) / (MaxSpeed - SteerReduceAt),
				0.0f, 1.0f);

			float dir = (fwdSpd >= 0.0f) ? 1.0f : -1.0f;

			float targetYaw = steerInput * dir * SteerSpeed
				* lowFactor * highFactor;


			angVel.y = glm::mix(angVel.y, targetYaw,
				glm::clamp(dt * 12.0f, 0.0f, 1.0f));
		}
		else
		{
			angVel.y = glm::mix(angVel.y, 0.0f,
				glm::clamp(dt * 7.0f, 0.0f, 1.0f));
		}
		...
		{
			glm::vec3 driftAngVel = body->GetAngularVelocity();

			float driftAssist = steerInput * 0.5f;
			driftAngVel.y += driftAssist;

			float driftYawLimit = SteerSpeed * 1.45f;
			driftAngVel.y = glm::clamp(driftAngVel.y, -driftYawLimit, driftYawLimit);
			body->SetAngularVelocity(driftAngVel);
		...
		if (m_has_cam)
		{
			...
			if (kW && !kS)
				targetOffset = -CamAccelPull;
			else if (kS && fwdSpd > 0.5f)
				targetOffset = CamBrakePush;
